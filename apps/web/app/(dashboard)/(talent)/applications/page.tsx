...
export default function ApplicationsPage() {
  ...
  const stats = data?.stats ?? { total: 0, active: 0, interviewing: 0, offers: 0 };
  ...
  const filtered = useMemo(() => {
    return applications
      ...
      .sort((a: CandidateApplicationListItem, b: CandidateApplicationListItem) => {
        switch (sortBy) {
          case "date_asc":
  ...
  const counts = useMemo(() => {
    ...
    for (const s of STATUS_FILTERS) {
      if (s.value !== "all") {
        c[s.value] = applications.filter((a: CandidateApplicationListItem) => a.status === s.value).length;
      }
    }
    return c;
  }, [applications]);
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="h-8 w-[140px] rounded-lg border-0 bg-muted/50 text-[12px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
        ...
        <CardContent>
          {filtered.length === 0 ? (
            ...
            <div className="space-y-2">
              {filtered.map((app: CandidateApplicationListItem, index: number) => {
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      className="group flex items-start gap-3 rounded-xl p-3 transition-colors hover:bg-accent/50"
                    >
