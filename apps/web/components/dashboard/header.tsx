...
function Breadcrumbs() {
  ...
  for (const seg of segments) {
    accumulated += `/${seg}`;
    const label = BREADCRUMB_MAP[accumulated] ?? seg.charAt(0).toUpperCase() + seg.slice(1);
            ...
            >
              ...
              >
                {!n.read ? (
                  ...
                  <Clock size={14} variant="Linear" className="text-muted-foreground" />
                )}
              </div>
              <div className="min-w-0 flex-1">
                <p className={cn("text-[13px] leading-tight", !n.read && "font-medium")}>
                  {n.title}
                </p>
