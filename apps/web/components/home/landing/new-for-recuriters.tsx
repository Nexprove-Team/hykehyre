...
import { CobeGlobe } from '@/components/global/globe'
...
export function NewForRecruiters() {
  return (
    <section className="relative mx-auto max-w-375 overflow-hidden py-20 lg:py-28">
      <motion.div
        initial="hidden"
        whileInView="visible"
      ...
      >
        ...
        <p className="mx-auto mt-4 max-w-xl text-xl text-neutral-400">
          Powerful tools that help you find, evaluate, and hire the best talent
          — faster than ever.
        </p>
      </motion.div>
...
function FeatureCard({
  ...
  return (
    <div
      className={cn(
        'group bg-background relative overflow-hidden border border-dashed px-8 pt-8 pb-6',
        className
      )}
    >
      {children}
...
function FeatureTitle({ className, ...props }: React.ComponentProps<'h3'>) {
  return (
    <h3
      className={cn(
        'text-foreground font-mono text-2xl font-semibold tracking-tight',
        className
      )}
      {...props}
    />
...
function SetupVisual() {
  return (
    <>
      <div className="bg-background outline-border relative mx-auto flex size-32 items-center justify-center rounded-full border-4 border-dashed shadow-xs outline outline-offset-4">
        <div className="from-foreground/20 via-foreground/5 absolute inset-0 z-10 scale-120 bg-radial to-transparent blur-xl" />
        <CustomTimerIcon className="fill-primary/90 size-14" />
      </div>

      <div className="relative mt-8 space-y-1.5 text-center">
        <FeatureTitle>Smart Filters</FeatureTitle>
        <FeatureDescription>
          Go beyond basic search with intelligent parameters.
        </FeatureDescription>
      </div>
    </>
  )
...
function UserBasedSecurity() {
  return (
    <>
      ...
      <div className="relative mt-8 space-y-1.5 text-center">
        <FeatureTitle>Verified Talent</FeatureTitle>
        <FeatureDescription>
          Pre-vetted candidate profiles you can trust.
        </FeatureDescription>
      </div>
    </>
  )
    ...
    <>
      <div className="min-h-32">
        <div className="absolute top-8 left-8 flex items-center gap-2">
          ...
          <div className="text-muted-foreground font-medium">4.5%</div>
        </div>
        <ReportsChartsSvg className="translate-x-[5%] scale-150 -rotate-2" />
      </div>
      <div className="relative z-10 mt-8 space-y-1.5 text-center">
        <FeatureTitle>Ranked Candidates</FeatureTitle>
        <FeatureDescription>
    ...
    <div className="grid h-full sm:grid-cols-2">
      <div className="relative z-10 space-y-6 py-8 ps-8 pe-2">
        <div className="bg-card outline-border/80 flex size-12 items-center justify-center rounded-full border shadow-xs outline outline-offset-2">
          <SquareMousePointerIcon className="text-primary/80 size-5" />
        </div>
        <div className="space-y-2">
          <FeatureTitle>Analytics Dashboard</FeatureTitle>
          <FeatureDescription>
            Track your hiring funnel metrics in real time.
          </FeatureDescription>
        </div>
      </div>
      {/* Dashboard Screen */}
  ...
  return (
    <div className="grid max-h-120 sm:grid-cols-2">
      <div className="space-y-6 pt-8 pb-4 pl-8 sm:pb-8">
        <div className="bg-card outline-border/80 flex size-12 items-center justify-center rounded-full border shadow-xs outline outline-offset-2">
          <GlobeIcon className="text-primary/80 size-5" />
        </div>
        <div className="space-y-2">
          <FeatureTitle>One-Click Distribution</FeatureTitle>
          <FeatureDescription>
            Post to multiple channels with a single click.
          </FeatureDescription>
        </div>
      </div>
...
function CustomLockIcon({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      <svg
        ...
        className="size-full"
      >
        {/* Ghost lock — always visible (faded fingerprint outline) */}
        <path
        ...
        {/* Solid lock — revealed by scan mask */}
        <g mask="url(#scan-reveal)">
        ...
        {/* Scan line — glowing bar that sweeps top to bottom */}
        <motion.rect
          x="0"
          width="212"
          height="3"
        ...
        <defs>
          {/* Reveal mask — follows the scan line */}
          <mask id="scan-reveal">
            <motion.rect
              x="0"
              y="0"
              ...
              fill="white"
              initial={false}
              animate={hovered ? { height: 143 } : { height: 0 }}
              transition={
                hovered ? { duration: 1.5, ease: 'linear' } : { duration: 0.3 }
              }
            />
          </mask>
          {/* Glow filter for the scan line */}
          <filter id="scan-glow">
